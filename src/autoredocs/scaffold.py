"""Scaffold templates for ``autoredocs init``.
...
# Auto-generated by: autoredocs init
...
ENV_EXAMPLE = """\
# autoredocs environment variables
...
def write_workflow(project_dir: Path, source: str = "./src") -> Path | None:
    """Create ``.github/workflows/autoredocs.yml`` for GitHub Pages deployment.

    Returns the path to the created file, or None if it already exists.
    """
    workflow_dir = project_dir / ".github" / "workflows"
    workflow_path = workflow_dir / "autoredocs.yml"
...
def update_gitignore(project_dir: Path) -> bool:
    """Append autoredocs entries to ``.gitignore`` if not already present.
    ...
    # Check if autoredocs entries are already present
    if "# autoredocs output" in existing:
        return False
